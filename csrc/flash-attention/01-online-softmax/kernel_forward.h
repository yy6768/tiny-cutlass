...
////////////////////////////////////////////////////////////////////////////////
// Online Softmax — row-wise safe softmax, in-place on P
//
// Grid : (num_queries, 1, num_batches * num_heads)
// Block: (kThreads, 1, 1)
// Smem : 2 * kNumWarps * sizeof(float)
//
// The first pass computes both row_max and row_sum with the online recurrence:
//   m_new = max(m, x)
//   d_new = d * exp(m - m_new) + exp(x - m_new)
//
// Two partial states (m0, d0) and (m1, d1) are merged as:
//   m = max(m0, m1)
//   d = d0 * exp(m0 - m) + d1 * exp(m1 - m)
//
// The second pass normalizes:
//   P[i, j] <- exp(P[i, j] - row_max) / row_sum
////////////////////////////////////////////////////////////////////////////////
template <
    typename scalar_t_,
    int kThreads_ = 256>
