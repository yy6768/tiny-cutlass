...
/*! \file
    \brief Three-stage Multi-Head Attention example with online softmax.

    Standard 3-stage attention launched as 3 independent kernels:
      MM0:     P = (Q @ K^T) * scale          (CUTLASS threadblock GEMM)
      Softmax: P[i,:] <- softmax(P[i,:])      (online raw kernel, in-place per row)
      MM1:     O = P @ V                       (CUTLASS threadblock GEMM)

    The full attention matrix P is still materialized in global memory between
    stages. This variant only changes the softmax stage from the 3-pass safe
    softmax baseline to a 2-pass online softmax.
...
struct Options {
  ...
  std::ostream & print_usage(std::ostream &out) const {
    out << "01_online_softmax_attention_test\n\n"
      << "Options:\n\n"
...
template <typename ArchTag_,
          ...
          int kKeysPerBlock_>
class TestbedOnlineSoftmaxAttention {
public:
...
public:
  ...
  TestbedOnlineSoftmaxAttention(Options &options_, uint32_t seed_ = 3080)
...
public:
  ...
  Result profile() {
    ...
    std::cout << "\nOnline Softmax Attention (3-stage, unfused):\n"
              << "====================================================\n"
              << "    {Sq, Sk, d, d_v, H, B} = {"
...
int run_attention(Options& options) {
  TestbedOnlineSoftmaxAttention<cutlass::arch::Sm80, kQueriesPerBlock, kKeysPerBlock> testbed(options);
