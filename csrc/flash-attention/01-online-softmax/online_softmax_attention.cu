...
/*! \file
    \brief Kernel entry for 01-online-softmax.

    The shared test harness owns allocation, cuDNN reference verification, and
    timing. This file only launches the 01 three-stage device path:

      MM0 -> online softmax -> MM1
*/
...
struct OnlineSoftmaxAttentionConfig {
...
cudaError_t run(
  ...
  if (problem.head_size_v > 64) {
    using Config = OnlineSoftmaxAttentionConfig<cutlass::arch::Sm80, 32, 128>;
    return launch_config<Config>(problem, tensors, workspace, stream);
  }

  using Config = OnlineSoftmaxAttentionConfig<cutlass::arch::Sm80, 64, 64>;
  return launch_config<Config>(problem, tensors, workspace, stream);
}

Kernel const kKernel = {
    "01-online-softmax",
    "Online Softmax Attention (3-stage)",
    workspace_bytes,
    can_run,
    run,
};
