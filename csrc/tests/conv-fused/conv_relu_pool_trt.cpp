...
struct Options {
  std::filesystem::path artifact_dir = "build/conv-fused/reference";
...
nvinfer1::Weights weights(std::vector<Element> const& values) {
...
bool build_engine(
  ...
  nvinfer1::IConstantLayer* bias1_layer = network->addConstant(
      ...
      weights(bias1));
  if (!input || !weight0_layer || !bias0_layer || !weight1_layer ||
      !bias1_layer) {
  ...
  nvinfer1::IPluginV3Layer* layer =
      network->addPluginV3(plugin_inputs, 5, nullptr, 0, plugin);
  if (!layer || !layer->getOutput(0)) {
    std::cerr << "TensorRT addPluginV3 failed\n";
    return false;
  }
  layer->getOutput(0)->setName("output_nhwc");
...
bool run_plugin(
  ...
  if (!context->setInputTensorAddress("input_nhwc", input.get()) ||
...
bool write_metrics(
  ...
  metrics << "  \"operator\": \"conv3x3_relu_pool_conv1x1_relu_pool\",\n";
  metrics << "  \"input_shape\": ["
          << options.batch << ", "
          << options.height << ", "
          << options.width << ", "
...
bool run(Options const& options) {
  conv::ConvReluPoolProblem problem{
      options.batch,
      options.height,
      options.width,
      options.channels,
  ...
  std::vector<Element> weight0;
  std::vector<Element> bias0;
  std::vector<Element> weight1;
  std::vector<Element> bias1;

  if (!read_binary(
          artifact_path(options, "input_nhwc.bin"),
          input,
          tensor_count(
              options.batch,
              options.height,
              ...
              options.channels)) ||
      !read_binary(
          artifact_path(options, "conv0_weight_krsc.bin"),
          weight0,
          size_t(options.hidden_channels) * 3 * 3 * options.channels) ||
      !read_binary(
          artifact_path(options, "conv0_bias.bin"),
          bias0,
          options.hidden_channels) ||
      !read_binary(
          artifact_path(options, "conv1_weight_krsc.bin"),
          weight1,
          size_t(options.output_channels) * options.hidden_channels) ||
      !read_binary(
          artifact_path(options, "conv1_bias.bin"),
          bias1,
          options.output_channels)) {
    return false;
  }
  ...
  DeviceBuffer<Element> d_output(output_count);
  ...
  Logger logger;
  std::unique_ptr<nvinfer1::ICudaEngine> engine;
  if (!build_engine(logger, options, weight0, bias0, weight1, bias1, engine)) {
    return false;
  }
